...
2. **Founder portraits are stopgaps.** Both are in, both are crops of casual photos:
   Abhimanyu on a boat in bright sun, Nishant indoors under warm light. They read fine as
   72px circles but they do not match each other, and they are not what a founder section
   on a paid product should carry. Worth two proper headshots on a consistent background
   when there is time.
...
- **The header wordmark is the real artwork**, not type. `peak-wordmark-ink.svg` and
  ...
  the nav goes solid.
- **Both portraits are cropped only, never retouched.** Square head-and-shoulders crops
  at 560px. On Nishant's I tried a white-balance correction and reverted it: the brightest
  thing in frame is a yellow wall, not a neutral reference, so auto-balancing pushed his
  skin green. The warm cast is the room. Abhimanyu's came from the square version of the
  boat photo, cropped to head and shoulders so the face survives at avatar size.
