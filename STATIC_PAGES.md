...
| `/careers` | `dist/careers/index.html` (roles + apply form) |
...
`/careers` is generated by `scripts/careers.mjs` from `src/content/careers.json`,
which is the single source of truth for open roles. To open, close or edit a
role, edit that JSON and rebuild. Each role becomes a section on the page plus
a `JobPosting` structured-data block for Google Jobs.

The apply form posts to a Google Apps Script web app. Put its URL in
`applyEndpoint` in the same JSON; while that is blank the page renders an email
fallback instead of the form. Setup steps and the script itself are in
`scripts/apps-script/`.

There is deliberately **no React route** for `/careers` — the static page is the
only implementation, so the form logic lives in one place. Link to it with a
plain `<a href="/careers/">`, never a router `<Link>`. To preview it locally:
